...
template <long int I>
void tooBig() {
  __attribute__((address_space(I))) int *bounds; // expected-error {{address space is larger than the maximum supported (4194282)}}
}
...
int main() {
  ...
  correct<0x3FFFE9>();
  tooBig<4194346>(); // expected-note {{in instantiation of function template specialization 'tooBig<4194346L>' requested here}}
