...
BuiltinCandidateTypeSet::AddPointerWithMoreQualifiedTypeVariants(QualType Ty,
  ...
  for (unsigned CVR = BaseCVR+1; CVR <= Qualifiers::CVRMask; ++CVR) {
    ...
    // Skip over propconst, it does not get involved in overload resolution
    if (CVR & Qualifiers::Propconst) continue;
...
ExprResult Sema::BuildCallToMemberFunction(Scope *S, Expr *MemExprE,
  ...
  QualType ResultType = Context.getConstPropagatedType(Method->getReturnType());
  ExprValueKind VK = Expr::getValueKindForType(ResultType);
  ResultType = ResultType.getNonLValueExprType(Context);
